...
while true; do
    while read -r line; do
        ...
        num1=$(echo "$line" | jq -r '.num1' 2>/dev/null)
        num2=$(echo "$line" | jq -r '.num2' 2>/dev/null)
        if [[ "$method" == "initialize" ]]; then
            ...
            id=$id
            #echo '{"jsonrpc":"2.0","status": "success", "result": $sum}"'
        
        elif [[ "$method" == "tools/list" ]]; then
            echo '{"jsonrpc":"2.0","id":"'"$id"'","result":{"tools":[{"name":"addition","description":"addition of two numbers.\n\nArgs:\n    num1, num2\n","inputSchema":{"properties":{"num1":{"title":"Number1","type":"string"},"num2":{"title":"Number2","type":"string"}},"required":["num1", "num2"],"title":"get_alertsArguments","type":"object"}}]}}'
            ...
            echo '{"jsonrpc":"2.0","status": "error", "message": "Invalid Input"}'
        fi
    done
done
